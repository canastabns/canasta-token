...
contract StakeForMoreSameToken is Ownable {
    ...
    /**
    * @notice rewardPerHour is 10000 because it is used to represent 0.0001, since we only use integer numbers
    * This will give users 0.01% reward for each staked token / hour
    */
    uint256 internal rewardPerHour = 10000;
    ...
    */
    ...
    /**
    * @notice A method to retrieve the stake date for a stakeholder.
    * @param _stakeHolder The stakeholder to retrieve the stake for.
    * @return uint256 The started at date.
    */
    function stakeStartedAtOf(address _stakeHolder) external view returns (uint256) {
        return stakes[_stakeHolder].startedAt;
    }
    ...
    */
    ...
    /**
    * @notice Calculate the balances for rewards.
    */
    function balanceForRewards() public view returns (uint256) {
        uint256 _totalRewards = totalRewards();
        uint256 _totalStakes = totalStakes();
        uint256 totalBalanceTokenInContract = token.balanceOf(address(this));
        return totalBalanceTokenInContract
        .sub(_totalStakes)
        .sub(_totalRewards);
    }
    ...
    */
    function _calculateReward(address _stakeHolder, uint256 _balanceTokenInContract) private view returns (uint256) {
        ...
        uint256 hoursElapsed = ((block.timestamp - userStake.startedAt) / 1 hours);

        uint256 reward = (hoursElapsed * userStake.amount) / rewardPerHour;
    ...
    */
    function distributeRewards() public onlyOwner {
        uint256 availableBalance = balanceForRewards();

        if(availableBalance > 0) {
            for (uint256 s = 0; s < stakeHolders.length; s += 1) {
                address stakeHolder = stakeHolders[s];
                uint256 reward = _calculateReward(stakeHolder, availableBalance);
    ...
    */
    function withdrawReward() external {
        ...
        uint256 balanceTokenInContract = token.balanceOf(address(this));
